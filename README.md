...
else — weekly limits, per-model buckets, usage credits — is one click away.
...
`Tests/` pins the parts of the response the app relies on, so a change to the
API shows up as a failing test rather than an empty popover.
